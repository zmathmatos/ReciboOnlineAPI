...
namespace Qsti.ReciboOnline.Infra.Repositories.Base
{
...

    {
        ...
        public IQueryable<TEntidade> ListarPor(Expression<Func<TEntidade, bool>> where)
        {
        ...
        public IQueryable<TEntidade> ListarPor(Expression<Func<TEntidade, bool>> where, params string[] includeProperties)
        {
            return Listar(includeProperties).Where(where);
        }
        ...
        public IQueryable<TEntidade> ListarEOrdenadosPor<TKey>(Expression<Func<TEntidade, bool>> where, Expression<Func<TEntidade, TKey>> ordem, bool ascendente = true, params string[] includeProperties)
        {
            return ascendente ? ListarPor(where, includeProperties).OrderBy(ordem) : ListarPor(where, includeProperties).OrderByDescending(ordem);
        }
        ...
        public TEntidade ObterPor(Func<TEntidade, bool> where, params string[] includeProperties)
        {
            return Listar(includeProperties).FirstOrDefault(where);
        }
        public TEntidade ObterPor(Func<TEntidade, bool> where)
        {
            return Listar().FirstOrDefault(where);
        }
        ...
        public TEntidade ObterPorId(TId id, params string[] includeProperties)
        {
            if (includeProperties.Any())
            {
                return Listar(includeProperties).FirstOrDefault(x => x.Id.ToString() == id.ToString());
        ...
        public IQueryable<TEntidade> ListarOrdenadosPor<TKey>(Expression<Func<TEntidade, TKey>> ordem, bool ascendente = true, params string[] includeProperties)
        {
            return ascendente ? Listar(includeProperties).OrderBy(ordem) : Listar(includeProperties).OrderByDescending(ordem);
        }
        ...
        public IQueryable<TEntidade> Listar(params string[] includeProperties)
        {
            IQueryable<TEntidade> query = _context.Set<TEntidade>();
